...
getInfoSNPs <- function(i,case.snps,ctrl.snps,patient.id){
  ...
  snps <- fread(case.snps$snps[i],data.table = FALSE,showProgress = FALSE,nThread = 2) %>% 
    filter(cov >= 10) %>% 
    filter(rsid %in% unique(ctrl.snps$rsid)) %>% 
    select(rsid,af,cov) %>% 
    mutate(af = if_else(condition = af < 0.5,true = 1 - af,false = af)) %>% 
...
for(i in seq_len(length(m))){
  ...
  message(unique(ptnt$patient))
  
  ctrl.snps <- ptnt %>% 
    filter(is.na(tc)) %>%
    pull(snps) %>% 
    fread(.,data.table = FALSE,showProgress = FALSE,nThread = 2) %>% 
    filter(cov >= 10) %>% 
    select(rsid,af,cov) %>% 
  ...
  out <- lapply(seq_len(nrow(case.snps)),getInfoSNPs, case.snps=case.snps, ctrl.snps=ctrl.snps, patient.id = unique(ptnt$patient)) %>% do.call(rbind,.)
