...
pileup_data <- '/BCGLAB/2020_signatures/synthetic_dilutions/pacbam/original_data'
...
for(i in seq_len(nrow(dil))){
  ...
  snps <- fread(snps.file,data.table = FALSE,showProgress = FALSE,nThread = nThread)
  ...
  pabs <- fread(pabs.file,data.table = FALSE,showProgress = FALSE,nThread = nThread)
...
savedata <- function(df,flag){
  ...
  write.table(tab,file = paste0(flag,'_coverage_per_sample.tsv'),quote = FALSE,row.names = FALSE,col.names = TRUE,sep = '\t')
  ...
  ggsave(filename = paste0(flag,'_coverage_per_sample.pdf'), plot = p, width = 210,height = 150,dpi = 300,units = 'mm',device = 'pdf')
  
}

savedata(df,flag = 'snps')
savedata(df,flag = 'pabs')